...
function parseFrom(from: string): { name: string; email: string } {
  ...
  return { name: "anlaşmalievlilik.net", email: from.trim() };
}

async function sendEmail(to: string, subject: string, html: string) {
  const from =
    process.env.EMAIL_FROM || "anlaşmalievlilik.net <noreply@anlasmalievlilik.net>";
...
export async function sendVerificationEmail(to: string, link: string) {
  ...
  return sendEmail(to, "E-postanızı doğrulayın — anlaşmalievlilik.net", html);
}
...
export async function sendPasswordResetEmail(to: string, link: string) {
  ...
  return sendEmail(to, "Parolanızı sıfırlayın — anlaşmalievlilik.net", html);
}
